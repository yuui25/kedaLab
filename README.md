...
利用シーンは商用ペネトレ・脆弱性診断、OSCP 等のペネトレ資格演習、各種演習環境での技術練習。
...
| 内部 VLAN に接続済み（IP のみ・認証情報あり問わず、内部ペネトレ開始時） | `00_Playbook/Internal_Network_Pentest_Flow.md` |

> ペネトレが初めての場合は `00_Playbook` に入る前に `06_Concepts/Pentest_Fundamentals.md` を参照することを推奨する。
...
**案件・演習が一区切りしたタイミング（または作業中断時）に追記する。**
...
③ ファイルを書く際は以下を確認する：
   a. ツール名が出てきたら「どこで使えるか（ペネトレ用Linuxディストリ標準/別途インストール/インターネット要）」を書いたか
   b. コマンドが「テスター端末側」か「ターゲット側」か明示されているか
