...
> **優先度「高」で権限昇格の糸口が見つからなかった場合は、そのまま優先度「中」の確認へ進む。**
> `sudo -l` が空・Capabilities なし・悪用できる SUID なし、という状態は珍しくない。
> 「中」のホームディレクトリ確認（`.bundle/config` / `.bash_history` / `.ssh/`）や内部サービス確認で認証情報・横移動経路が見つかることが多い。
