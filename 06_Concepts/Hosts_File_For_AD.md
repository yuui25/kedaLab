...
SPN は `CIFS/dc01.example.local` のようなホスト名ベースの形式で、IP アドレスでは要求できない。
...
`/etc/hosts` に `192.0.2.10  dc01.example.local  dc01` を登録しておくと、
...
ドメイン名が分からないと bind すらできない。さらに `ldap://dc01.example.local` のような URI で接続するツールも多い。
...
```
[IP]  [hostname]                # 例: dc01
[IP]  [domain.tld]              # 例: example.local
[IP]  [hostname].[domain.tld]   # 例: dc01.example.local
```
...
| `nxc smb [IP]` の出力 | `(name:DC01) (domain:example.local)` |
