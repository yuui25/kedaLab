...
| Technique ID | 技術名 | kedalab ファイル |
|------|-------|--------|
| T1562.001 | Impair Defenses: Disable or Modify Tools（AMSI） | `04_Post_Access_Windows_AD/Enumeration_Checklist.md`（Step 8 AMSI バイパス） |
| T1562.001 | Impair Defenses: Disable or Modify Tools（BYOVD） | `04_Post_Access_Windows_AD/BYOVD.md` |
...
- 1技術が複数 Technique にまたがる場合は両方の行に記載してよい
- Sub-Technique がある場合は Sub-Technique ID（`T1078.001` 等）を優先して使う
- ATT&CK ID と kedalab ファイルが N:N になっても良い。完全な分類より「引きやすさ」を優先する
