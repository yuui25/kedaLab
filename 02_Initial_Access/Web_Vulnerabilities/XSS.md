...
- SQLi（同じ入力フィールドの脆弱性・バイパス手法が重複） → `SQLi.md`
- SSRF（入力値がサーバー側リクエストになる経路） → `SSRF.md`
- LLM 出力経由の XSS（Improper Output Handling・LLM が生成した内容が未検証で表示される） → `../../06_Concepts/AI_ML/Generative_AI/LLM_Attacks.md`
